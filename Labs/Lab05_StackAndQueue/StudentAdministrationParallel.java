...
class Student {
    ...
    public Student(String name, int giveDocuments, int index, int takeDocuments) {
        this.name = name;
        this.giveDocuments = giveDocuments;
        this.index = index;
        this.takeDocuments = takeDocuments;
    }
}
...
public class StudentAdministrationParallel {
    public static void main(String[] args) {
        ...
        scanner.nextLine();
        LinkedQueue<Student> giveDocuments = new LinkedQueue<>();
        LinkedQueue<Student> index = new LinkedQueue<>();
        LinkedQueue<Student> takeDocuments = new LinkedQueue<>();
        for (int i = 0; i < n; i++) {
            ...
            if (activity1 == 1)
                giveDocuments.enqueue(student);
            else if (activity2 == 1)
                index.enqueue(student);
            else if (activity3 == 1)
                takeDocuments.enqueue(student);
            scanner.nextLine();
        }
        while (!giveDocuments.isEmpty() || !index.isEmpty() || !takeDocuments.isEmpty()) {
            if (!giveDocuments.isEmpty()) {
                Student student = giveDocuments.dequeue();
                student.giveDocuments = 0;
                if (student.index == 1)
                    index.enqueue(student);
                else if (student.takeDocuments == 1)
                    takeDocuments.enqueue(student);
                else
            ...
            if (!index.isEmpty()) {
                ...
                student.index = 0;
                if (student.takeDocuments == 1)
                    takeDocuments.enqueue(student);
                else
                    System.out.println(student.name);
            }
            if (!takeDocuments.isEmpty()) {
                Student student = takeDocuments.dequeue();
                student.takeDocuments = 0;
                System.out.println(student.name);
            }
        }
    }
