...
за кое ќе треба да одговорите на Q прашања од видот „која е сумата на степените во поддрвото на избран јазол“.
...
for which you will need to answer Q questions of the type "what is the sum of degrees in the subtree of a selected node".
...
public class SumOfDegrees {
    ...
    public static int countChildren(BNode<String> node) {
        if (node == null)
            return 0;
        int counter = 0;
        if (node.left != null)
            counter += 1 + countChildren(node.left);
        if (node.right != null)
            counter += 1 + countChildren(node.right);
        return counter;
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i < n + q; i++) {
            ...
            if (type.equals("add")) {
                ...
                System.out.println(countChildren(findNode(tree.root, parent)));
            }
        }
    }
}
